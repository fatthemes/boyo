...
class Boyo_CSS_Loader {
	...
	private function supported_browsers_string( $min_width = '' ) {
		...
		if( ! empty( $min_width ) ) {
            //$min_width_string = ', ';
			$min_width_string .= ' and ( min-width: ' . $min_width . ')';
		}
		$output = 'only print' . $min_width_string . ',';
		$output .= 'only all and (-ms-high-contrast: none)' . $min_width_string . ',';
		$output .= 'only all and (-ms-high-contrast: active)' . $min_width_string . ',';
		$output .= 'only all and (pointer: fine)' . $min_width_string . ',';
		$output .= 'only all and (pointer: coarse)' . $min_width_string . ',';
		$output .= 'only all and (pointer: none)' . $min_width_string . ',';
		$output .= 'only all and (-webkit-min-device-pixel-ratio:0) and (min-color-index:0)' . $min_width_string . ',';
